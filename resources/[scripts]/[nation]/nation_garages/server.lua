...
RegisterCommand('car',function(source,args,rawCommand)
	...
	if user_id then
		...
		if vRP.hasPermission(user_id,"dono.permissao") or vRP.hasPermission(user_id,"manager.permissao") then
			if args[1] then
	...
	if user_id then
		if args[1] and parseInt(args[2]) > 0 then
			...
			if myvehicles[1] then
				if config.getVehicleType(tostring(args[1])) == "exclusive" and not vRP.hasPermission(user_id,"manager.permissao") then
...
RegisterCommand('hash',function(source,args,rawCommand)
    local user_id = vRP.getUserId(source)
    if vRP.hasPermission(user_id,"mindmaster.permissao") or vRP.hasPermission(user_id,"manager.permissao") then
