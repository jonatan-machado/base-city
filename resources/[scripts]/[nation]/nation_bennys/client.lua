...
RegisterNUICallback("pagar",function(data)
	if cart["total"] and func.checkPayment(cart["total"]) then
		...
		TriggerEvent('progress',5000,"aplicando")
		Wait(5000)
		TriggerEvent("Notify","sucesso","Sucesso","Modificações aplicadas com <b>sucesso</b>.",7000)
		vRP.stopAnim(false)
		myveh = getAllVehicleMods(vehicle)
	...
	if data then
		if data.type == "reparar" then
			...
			if func.repairVehicle(vehicle,damage) then
				...
				myveh.damage = 0.0
				vRP.stopAnim(false)
				TriggerEvent("Notify","sucesso","Sucesso","Veículo reparado com <b>sucesso</b>.",7000)
...
function isVehicleTooFar(veh)
	Citizen.CreateThread(function()
		while nui do
			...
			if distance > 6 then
				fclient.closeNui()
				TriggerEvent("Notify","aviso","Aviso","Você se afastou muito do veículo.",7000)
			end
			Citizen.Wait(500)
		end
	end)
...
function camControl(c)
	...
	elseif c == "dianteira" or c == "wheel-accessories" or  c == "wheel-colors" or c == "sport" or c == "muscle" or c == "lowrider"  or c == "highend" or c == "suv" or c == "offroad" or c == "tuner" or c == "import" then
