...
function API.checkPayment(amount)
    ...
    if not vRP.tryPayment(user_id, tonumber(amount)) then
        TriggerClientEvent("Notify",source,"negado","Negado","Você não possui dinheiro suficiente.",7000)
        return false
    end
    TriggerClientEvent("Notify",source,"aviso","Aplicando Modificações","Você deve aguardar até <b>as modificações serem aplicadas.</b> Você pagou <b>$"..tonumber(amount).." dólares</b>.",7000)
    return true
end
