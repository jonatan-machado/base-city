...
PdeF = Tunnel.getInterface("thor_transportadora",PdeF)
--==================================================== * VARIÁVEIS DE CONFIGURAÇÃO * ====================================================--

local limiteCofre = 2 --> Máxima quantidade de sacos que se pode pegar em cada cofre. (NUNCA deixar valor maior que 6)
local quantPguardar = 1 --> Quantidade de sacos que o player deve guardar no carro forte / (NUNCA deixar valor maior que 6)
...
local pag_min = 300 --> Pagamento minimo por saco
local pag_max = 900 --> Pagamento maximo por saco
            ...
            if Vdist(x,y,z,-9.98,-657.4,33.45) <= 2 and wx == true then
                if IsControlJustPressed(0,38) then
                    if op then
                        ...
                        TriggerServerEvent('pagamento', pagamento)
