...
function open(vehicle,model,money)
	SetNuiFocus(true, true)
	SendNUIMessage({ action = true, fuel = GetVehicleFuelLevel(vehicle), vehicle = model, money = money, link = Config.linkimages, format = Config.imageformat, precolitro = Config.precolitro })
end
		...
		while not isFueling and ((isNearPump and GetEntityHealth(isNearPump) > 0) or (GetSelectedPedWeapon(ped) == 883325847 and not isNearPump)) do
			if IsPedInAnyVehicle(ped) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped),-1) == ped then
				...
				elseif isNearPump then
					...
					if IsControlJustReleased(0,38) then
						TriggerServerEvent('nation_fuel:pagamento',parseInt(300),true)
					end
				end
			end
			Citizen.Wait(4)
...
RegisterCommand('gasosa',function()
	local ped = PlayerPedId()
	local vehicle = GetPlayersLastVehicle()
